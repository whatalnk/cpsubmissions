...
// Submitted: 2018-01-11 08:10:27 +0000 UTC ( https://atcoder.jp/contests/wn2017_1/submissions/1963004 ) 
...
fn i16_to_u8(x: i16) -> [u8; 2] {
    let v: [u8; 2] = unsafe { std::mem::transmute(x) };
    v
}

fn u8_to_i16(x: [u8; 2]) -> i16 {
    let v: i16 = unsafe { std::mem::transmute(x) };
    v
}

fn subtract(v1: &Vec<i16>, v2: &Vec<i16>) -> Vec<i16> {
    let v = v1.iter()
        .zip(v2.iter())
        .map(|(a, b)| a - b)
        .collect::<Vec<i16>>();
...
fn add(v1: &Vec<i16>, v2: &Vec<i16>) -> Vec<i16> {
    let v = v1.iter()
        .zip(v2.iter())
        .map(|(a, b)| a + b)
        .collect::<Vec<i16>>();
...
fn make_tree(freq_table: HashMap<&i16, usize>) -> Node {
...
impl Huffman {
    ...
    fn encode(&self, data: &[i16]) -> String {
        let mut encoded = String::new();
        for x in data {
    ...
    fn decode(&self, encoded: String, size: usize) -> Vec<i16> {
...
impl Tree {
    fn write_tree(&mut self, node: &Node) {
        match node.code {
            ...
            Some(c) => {
                self.out.push(1);
                self.out.append(&mut i16_to_u8(c).to_vec());
            }
        }
    }
    ...
    for (_, v) in &fileinfos {
        for x in &v[..1] {
            ...
            let data_u16 = data.chunks(2)
                ...
                .collect::<Vec<u16>>();
            // u16 to i16
            let data_i16 = data_u16.iter().map(|x| *x as i16).collect::<Vec<i16>>();
            data_list.entry(&x.filename).or_insert(data_i16);
        }
        for x in &v[1..] {
            let path_in = Path::new(&directory_name).join(&x.filename);
            let mut data = Vec::new();
            ...
            match data_list.entry(&v[0].filename) {
                ...
                Entry::Occupied(e) => {
                    let v0 = e.get();
                    data_subtracted = subtract(&data_i16, v0);
                }
            }
