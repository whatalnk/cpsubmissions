...
// Submitted: 2017-08-09 13:13:48 +0000 UTC ( https://atcoder.jp/contests/arc080/submissions/1499125 ) 
...
fn main() {
    ...
    let n = usize::from_str(buf.trim()).unwrap();
    buf = String::new();
    stdin.read_line(&mut buf).ok();
    let arr: Vec<i64> = buf.split_whitespace()
    ...
    for (i, a) in arr.iter().enumerate() {
        h.insert(a, i);
        if i % 2 == 0 {
            st_even.update(i, a.clone());
        } else {
            st_odd.update(i, a.clone());
        }
    }
